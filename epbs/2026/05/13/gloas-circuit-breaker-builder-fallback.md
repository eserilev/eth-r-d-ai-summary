# Gloas Circuit Breaker and Builder Fallback Mechanisms

**Channel:** epbs | **Date:** 2026-05-13

## Summary

The discussion centers on how circuit breaker mechanisms for fallback block building will work in the post-Gloas Ethereum environment. The core question is whether the current circuit breaker that switches to local block building will be preserved. While some participants initially suggested maintaining the simple approach of falling back directly to local building when builders fail to reveal payloads, potuz advocated for a more sophisticated approach that can ban specific problematic builders rather than abandoning all external building immediately.

The conversation revealed that there are multiple fallback mechanisms being considered, including reverting to only trusted builders, only local building, or only P2P bids depending on the failure scenario. The first line of defense would be blacklisting individual builders, which works well for trusted builders but presents challenges with P2P builders due to potential sybil attacks where builders can easily create new public keys. A key technical challenge identified is the difficulty in distinguishing whether a bid came from a trusted builder or from P2P just by examining the bid itself.

The participants referenced detailed interop discussions and documentation about these fallback mechanisms, indicating this is an active area of research with documented proposals, though the specific implementation details for post-Gloas remain to be finalized.
