# Payload Attestation Equivocation Prevention in Blocks

**Channel:** epbs | **Date:** 2026-05-05

## Summary

jtraglia raised a concern about payload attestation equivocation prevention, noting that malicious proposers could theoretically include two `PayloadAttestation` aggregates in the same block with the same validator's bit set in both, since `process_payload_attestation` checks them independently. He suggested this indicates buggy or malicious behavior and proposed making blocks invalid if they contain conflicting votes from the same validator index, arguing it could prevent potential split views depending on client implementations.

potuz disagreed with treating this as a significant issue, explaining that equivocations in the same block are irrelevant since they're processed in order with the last one taking precedence, and can't cause split views. He noted that split views from equivocations already exist regardless of proposer actions, and emphasized that conditions invalidating blocks should be avoided unless absolutely necessary since they represent implementation bugs.

The discussion remained unresolved with differing viewpoints. jtraglia created PR #5209 to implement the proposed equivocation check but indicated willingness to close it if others disagree. The core tension centers on whether preventing this edge case justifies adding block invalidation conditions versus accepting the existing equivocation handling mechanisms.
