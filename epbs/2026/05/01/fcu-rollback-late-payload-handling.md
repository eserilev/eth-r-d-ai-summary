# FCU Sending Strategy and Rollback Prevention

**Channel:** epbs | **Date:** 2026-05-01

## Summary

The discussion focuses on a strategy to minimize rollbacks in the Ethereum consensus layer by changing when FCU (forkchoiceUpdated) calls are sent to the execution layer. Potuz proposes sending FCU calls **only** when proposing blocks (with payload attributes) rather than on every slot advance or attestation, which would make the FCU serve more as a hint for block production rather than causing rollbacks. This approach would mean FCUs are only sent during payload insertions or reorgs, but not on routine slot advances.

The motivation stems from concerns that rollbacks could break applications, as evidenced by a bug encountered by Lighthouse when the first block was empty. M.kalinin notes that most execution layer clients can only propose blocks on top of the chain head anyway, so they would need to handle rollbacks regardless. However, the goal is to avoid unnecessary rollbacks for user experience reasons rather than technical limitations.

The conversation reveals some tension around the broader implications, with m.kalinin pointing out that ePBS (enshrined Proposer-Builder Separation) and honest reorgs naturally cause rollbacks anyway. An open consideration emerges around the expectation that dapps shouldn't be consuming data from nodes serving validator duties, suggesting this rollback prevention strategy may be most relevant for specific node configurations.
