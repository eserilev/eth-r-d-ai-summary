# PTC Duplicate Validators Discussion

**Channel:** epbs | **Date:** 2026-05-06

## Summary

The discussion centers on whether duplicate validators should be allowed in PTC (Payload Timeliness Committee). Initially there was confusion about the current specification, with jtraglia clarifying that his previous statement at interop was incorrect based on an old spec. The participants confirmed that duplicate validators are indeed possible and allowed in the current implementation due to the balance-weighted selection algorithm that wraps around when filling the required 512 committee positions, particularly in smaller chains where there may be fewer total validators.

The conversation revealed that while duplicates are theoretically possible on mainnet (if all random dice throws were 0), this scenario is extremely unlikely in practice. The main concern raised by nflaig relates to a gossip rule in the PTC payload reorg mechanism that ignores messages after the first valid one from a validator index, though this issue primarily affects small devnets rather than mainnet operations.

The group concluded that duplicate PTC members, while possible, don't pose a consensus danger and are mainly relevant for small development networks. No specific action items were identified, and the current specification appears to handle this edge case appropriately.
