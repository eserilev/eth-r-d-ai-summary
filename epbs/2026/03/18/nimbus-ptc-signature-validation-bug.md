# Nimbus PTC Signature Validation Bug

**Channel:** epbs | **Date:** 2026-03-18

## Summary

Terence Chain reported a bug where Nimbus validators were producing blocks with invalid PTC (Payload Transaction Committee) signatures across multiple slots, affecting Nimbus versions v26.3.0-12730e and v26.3.0-1077b0. The issue was identified in 12 different blocks proposed by various validators between slots 95158 and 96045.

Tomi investigated and found the root cause: Nimbus had not properly upgraded their `is_invalid_indexed_payload_attestation` function, which was still using version 1.6.0-alpha.6. This caused the function to compute the signing domain against `GENESIS_EPOCH` instead of using the actual epoch from the `PayloadAttestationData`, resulting in invalid signatures.

The bug appears to occur specifically when Gloas (likely a network upgrade) isn't activated at Genesis, and Tomi noted that the current specification tests don't cover this particular scenario. Potuz mentioned that this could potentially cause a network split on PTC validation, highlighting the severity of the issue.
