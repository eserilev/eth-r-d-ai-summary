# Gloas Genesis Definition Issues and Fixes

---

## 2026-05-05 (epbs)

The discussion centers around issues with Gloas genesis definition in Ethereum client implementations, specifically around inconsistencies between the consensus specification and the ethereum-genesis-generator used by Kurtosis. The core problem is that while the specification defines genesis blocks to include execution payload bids, the genesis generator was not implementing this correctly, leading to mismatches between genesis block roots and state latest block roots.

Client implementations had varying responses to a proposed fix (PR#77): Lighthouse and Nimbus accepted it, while Prysm, Lodestar, Teku, and Grandine rejected it with validation errors. However, participants noted that all clients were passing consensus spec tests, suggesting they had implemented various "hacks" or workarounds during interop testing to handle the inconsistent genesis generation. The discussion revealed that some clients like Prysm had removed these hacks after interop, while others maintained them.

The consensus emerged that the specification itself is correct in requiring execution payload bids in genesis blocks, and the issue lies in aligning the genesis generator with this specification. Sproul indicated they would investigate the other client implementations to understand why they're rejecting the correct genesis format, while several developers acknowledged they had implemented temporary fixes that may need to be revisited.
