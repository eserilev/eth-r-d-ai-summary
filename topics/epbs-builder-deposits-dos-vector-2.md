# Builder Deposit Queue and DoS Attack Prevention

---

## 2026-05-08 (epbs)

The discussion centers on preventing DoS attacks in Ethereum's proposed builder deposit queue system. Potuz raises concerns that malicious builders can easily clog the deposit queue since signature verification limits make it cheap to DoS others - with current parameters, a builder could fill the entire queue for just $37M, and queuing doesn't fundamentally solve the signature verification bottleneck. The team identifies a core tradeoff: if signature verification limits per slot are small (N), it's cheap to DoS; if large, nodes still can't process the deposits efficiently.

Several mitigation strategies are proposed and debated. The primary solution gaining traction is significantly increasing the builder withdrawal delay (originally 4096 epochs, reduced to 64, now proposed to increase again) since attack cost scales as `min_deposit × signatures_per_epoch × withdrawal_delay`. Alternative ideas include charging exit fees, disabling builder exits entirely, or moving signature verification to the execution layer with gas costs. Performance testing reveals concerning results - 1500 builder deposits already stress the network on M1 Pro hardware, and 8192 deposits take 640ms-2.1s depending on validity, suggesting mid-tier hardware could struggle significantly.

The discussion concludes with agreement on immediate actions: increase the minimum builder withdrawal delay, raise SSZ limits to prevent execution layer blocking, and potentially implement EIP-7688 (progressive containers) in the next devnet in 2 weeks. However, there's consensus that proper stress testing with 8192+ deposits should wait until the ePBS implementation is more stable, and client teams should focus on optimizing large deposit handling in parallel.
