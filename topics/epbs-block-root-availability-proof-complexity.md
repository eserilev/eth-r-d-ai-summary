# EPBS Block Root Availability and Proof Complexity

---

## 2026-03-17 (epbs)

This discussion centers on a technical challenge introduced by EPBS (Enshrined Proposer-Builder Separation) where not all beacon block roots will be available in the EIP-4788 beacon roots contract on-chain. Specifically, parent roots of blocks with empty payloads won't be stored, which complicates proof construction for applications that need to prove specific operations like withdrawals occurred in particular blocks.

The main impact is on proof complexity and gas costs. pk910 calculated that proving withdrawals would require 36 sibling hashes (22 for beacon root traversal + 14 for withdrawal root) compared to the current 16 hashes, effectively doubling the proof size and costs. However, they identified that adding an `execution_payload_root` to the bid could solve this issue, and potuz confirmed this is technically feasible since bids are consensus layer only. Additionally, fixing the beacon roots contract could reduce proofs to just 14 hashes, making them cheaper than current implementations.

The discussion concluded that while EPBS introduces proof complexity challenges, there are viable workarounds through extended proofs via state roots, and the withdrawal proving issue specifically could be addressed by including execution payload roots in bids. ralexstokes emphasized that any historical data can still be proven by extending proofs by a few additional hashes when direct block roots aren't available in the 4788 contract.

**Participants:** ajsutton, dgusakov, nixo.eth, pk910, potuz, ralexstokes
