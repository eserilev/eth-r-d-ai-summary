...
The discussion centers on whether the Gloas Genesis handling specification should be formally specified, with jtraglia advocating for clarification while potuz and brechy oppose it. Jtraglia argues that if clients are expected to handle gloas genesis for devnets and Kurtosis, it needs proper specification, and there won't be a release until this is resolved. The technical issue involves bid processing assertions where having `0x00` in the genesis state doesn't fully solve the problem - if a block at slot 1 has a bid with `parent_hash = 0x00`, it creates the same situation where the genesis slot appears "full".

Potuz questions the necessity of specifying this for mainnet Ethereum, suggesting that devnet implementations don't require formal specification and that leaving the first block empty would be acceptable. He's concerned that adding this specification would make the actual Ethereum spec less readable without sufficient justification. A potential alternative discussed involves generating the state so that `parentIsFull` returns false, though this introduces its own confusion since the genesis block's parent is not actually full.

The discussion remains unresolved, with jtraglia acknowledging both sides of the argument and suggesting they need to give another contributor (Sproul) time to respond, as he apparently considers this specification important. The core tension is between practical implementation needs for testing environments versus keeping the main specification clean and focused on mainnet requirements.

**Participants:** brechy, jtraglia, potuz, tbenr
...
The discussion centers on inconsistencies in the Gloas genesis definition between the current ethereum-genesis-generator used by Kurtosis and the consensus specification. Sproul identified that the genesis generator doesn't properly include the execution payload bid in the genesis block, while the spec requires it. This issue was initially worked around with client-specific hacks during interop testing.

When Sproul submitted PR #77 to align the genesis generator with the spec, it caused failures in several clients (Prysm, Lodestar, Teku, Grandine) that were expecting the old behavior, while Lighthouse and Nimbus worked correctly. The failures manifested as mismatched block roots between the genesis block and genesis state's latest block header. Several developers confirmed their clients pass consensus spec tests after removing genesis-specific hacks, suggesting the issue is with the genesis generator rather than the specification itself.

The consensus appears to be that the specification is correct and the genesis generator needs to be fixed to match it, but some clients may have enshrined workarounds that now need to be removed. Sproul plans to investigate the failing clients further, while the community needs to coordinate removing client-specific genesis hacks to achieve consistent behavior across implementations.
...
The discussion centers around a fix to the Gloas genesis configuration in the Ethereum package. After jtraglia closed an unspecified issue, nflaig referenced "gloas genesis final-v3-final," suggesting multiple iterations of the genesis configuration.

Barnabasbusa announced that the Gloas genesis has been fixed in the Ethereum package, but warned that this change is expected to cause broken startup behavior for most consensus layer (CL) clients. However, tersec questioned what "fixed" actually means in this context, leaving the specific nature of the fix unclear and suggesting there may be ongoing uncertainty about the resolution.
...
```
[2026-05-06T11:29:02.135000+00:00] jtraglia: Closed this.
[2026-05-06T20:16:12.872000+00:00] nflaig: gloas genesis final-v3-final
[2026-05-06T21:41:04.410000+00:00] barnabasbusa: gloas genesis is now fixed in ethereum package. expect to see broken startup for most cl clients.
[2026-05-06T22:26:36.066000+00:00] tersec: what does "fixed" mean this time?
```
