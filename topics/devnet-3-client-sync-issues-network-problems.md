# Devnet-3 client sync issues and network problems

---

## 2026-05-11 (epbs)

Multiple Ethereum consensus clients (Prysm, Lighthouse, and Grandine) are experiencing sync issues on devnet-3, with only Lodestar and Teku successfully maintaining network operations. The root cause appears to be related to payload envelope serving problems and inconsistent validation logic between clients. Potuz identified that Prysm is stuck on an old finalized checkpoint due to a consensus bug around slot 15742, where Prysm rejected a block due to payload withdrawal mismatches, while other clients accepted it and continued building the chain.

The technical investigation revealed that Prysm and Lighthouse are both failing to sync past block 23443 because they cannot obtain the necessary payload envelopes from peers. Prysm is implementing stricter validation that treats inconsistent payload data as malicious behavior and bans peers, while Lodestar appears to have more lenient validation that allows it to continue syncing. Potuz suggested this validates his earlier recommendation to abandon the current payload serving approach entirely in favor of EIP 8237, expressing frustration that this path was previously dismissed.

The immediate next steps involve waiting for further analysis to determine whether Prysm's strict validation is correct or if the validation logic needs adjustment. The network is currently being supported by additional Lodestar nodes deployed to help with sync issues, but a fundamental fix to the payload serving mechanism or validation inconsistencies between clients appears necessary to resolve the ongoing problems.
