...
The discussion centers on implementing "honest PTC reorgs" where proposers can reorganize blocks based on Payload Timeliness Committee (PTC) votes. The main issue addressed is that under current fork choice rules, a proposer who has seen their required data columns might build on a full block even when the PTC overwhelmingly votes that data availability (DA) is missing. This leads to unnecessary reorgs when attesters in the next slot likely agree with the PTC's negative assessment.

Potuz explains that the proposed changes allow proposers to act rationally by reorging payloads when receiving majority negative PTC votes, preventing the chain from missing an extra payload in the next slot. The mechanism works through the existing `should_extend_payload` function, which returns false when PTC votes against payload availability, causing attesters to naturally follow the proposer's reorg decision. Francesco clarifies that reorgs should only occur with majority negative votes (not just absent votes) to ensure attesters will share the negative opinion by including those votes in blocks.

The team opened multiple PRs to address different scenarios: PR #5186 for DA-based reorgs, PR #5210 for payload availability reorgs, and PR #5212 for dual deadlines enforcement. An important clarification was made that these changes only affect proposer behavior - there's no proposal to force attesters to reorg based on PTC votes, as they already follow the existing fork choice rules that naturally align with honest proposer reorgs.

**Participants:** bharath.vedartham, fradamt, m.kalinin, potuz
