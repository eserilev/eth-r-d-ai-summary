...
The discussion centers on a gas limit issue encountered during BatchDeposit contract deployment in an Ethereum testnet environment using Kurtosis with Geth and Prysm. When the genesis gas limit is set to high values (150-200M), the contract deployment fails with "contract creation code storage out of gas" error, consuming exactly 2,000,000 gas, while the same transaction succeeds with ~920k gas when using the default genesis gas limit.

barnabasbusa suggests this may be related to issue 8073, explaining that higher gas limits cause more dynamic pricing in batch creation. As a workaround, terencechain found that a genesis gas limit of 120M works successfully, allowing testing of ~4000 deposits per slot with good performance from their cache and batch verification system.

The team identified that higher gas limits would require correspondingly higher transaction gas limits, though this would slow network catchup. An action item was noted for "pk" to investigate the underlying issue further, while terencechain expressed interest in testing invalid signatures within the deposits.
...
The discussion centers around a gas limit validation issue in the Ethereum consensus specs where the current p2p check incorrectly validates `bid.gas_limit == proposer_preferences.gas_limit` instead of checking the gas limit based on the parent block's gas limit using EIP-1559 calculations. The participants identified an inconsistency in the spec: proposer preferences contain the desired/target gas limit, but bids should contain the actual calculated gas limit that follows EIP-1559 transition rules, which can differ from the target when gas limits are being adjusted gradually over blocks.

The group reached consensus on moving gas limit control from the Execution Layer (EL) to the Consensus Layer (CL) by deprecating EL gas limit flags and passing the gas limit via `engine_forkchoiceUpdatedV*`. However, they discovered a complication: the beacon state no longer stores the actual gas used, making EIP-1559 validation in the CL more problematic. Two potential solutions were proposed: making bids contain the actual gas limit with CL verification of EIP-1559 rules, or having bids contain target gas limits while storing actual values in the beacon state.

Action items include discussing the issue in ACDT (All Core Devs Testing), with potuz opening two PRs to address the different approaches and nflaig submitting an execution-apis PR to pass gas limits via forkchoice updates. The participants also suggested adding a maximum gas limit cap for safety, similar to EIP-3756, to prevent accidentally damaging the network during gas limit increases.

**Participants:** barnabasbusa, bharath.vedartham, jtraglia, nero_eth, nflaig, potuz, tbenr, tersec
