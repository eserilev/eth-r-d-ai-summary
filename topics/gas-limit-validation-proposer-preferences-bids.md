# Gas limit validation in proposer preferences and bids

---

## 2026-05-11 (epbs)

The discussion centers on a bug in the gas limit validation for execution payload bids in the Gloas specification. The current p2p gossip check incorrectly validates `bid.gas_limit == proposer_preferences.gas_limit`, but this fails during gas limit transitions since the bid should contain the calculated gas limit based on EIP-1559 rules (derived from parent block gas limit), while proposer preferences contain the target/desired gas limit. Several participants confirmed that consensus layer clients already implement EIP-1559 calculations for validating builder payloads, so the fix should involve using the proper gas limit transition calculation in gossip validation.

The group identified a broader architectural issue where both consensus layer and execution layer clients can set conflicting gas limit preferences, leading to potential bid rejections. There's consensus that gas limit control should be consolidated in the consensus layer, with the gas limit passed via `engine_forkchoiceUpdatedV*` rather than relying on execution layer flags. However, implementing EIP-1559 validation in the consensus layer is complicated by the fact that actual gas usage is no longer stored in the beacon state after recent changes.

Potuz committed to opening two PRs to address the immediate issue: one making bids contain the actual calculated gas limit (enabling CL-based EIP-1559 verification), and another where bids contain target gas limits with actual usage stored in beacon state. The team agreed to discuss the broader architectural changes in the ACDT call, with an execution API PR already submitted to enable gas limit passing through forkchoice updates.
