...
nc1234 proposed simplifying the proposer boost reorg mechanism following recent changes to the forkchoiceUpdated (fcU) call referenced in consensus specs pull request #5174. The suggestion received positive support from multiple contributors.

sproul explained that the original restriction existed because the execution layer (EL) could be ahead of the consensus layer (CL), making fcU calls appear like reversions. They suggested a potential solution of stopping fcU sends during finalized sync and referenced a related GitHub issue for additional context. jtraglia also expressed support for the simplification.

However, potuz noted that this change has already been implemented, suggesting the proposed simplification may have already been completed. No further action items or open questions were explicitly discussed in this conversation.

**Participants:** jtraglia, nc1234, potuz, sproul
