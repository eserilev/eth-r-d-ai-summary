# Independent CL/EL Sync EIP Impact on Nimbus

---

## 2026-04-23 (epbs)

The discussion centers around an EIP for Independent CL/EL Sync and its potential impact on the Nimbus client. Potuz inquired whether this EIP would negatively affect Nimbus, particularly regarding independent range syncing. The Nimbus team (0xshum and tersec) confirmed they should be fine with the core syncing changes as long as the Envelope is not required in state transition, though there are concerns about peripheral effects.

A key technical debate emerged around the EIP's use of SHA256 instead of SSZ hashing, with tersec pointing out that execution layer clients already have SSZ implementations (citing Geth and Erigon's light clients). However, potuz explained they chose SHA256 to avoid "infinite bike-shedding arguments" with EL developers, despite SSZ being simpler for the consensus layer. The main concern for Nimbus is that they store execution payloads directly rather than using the "blinded" approach like Lighthouse, which could create complications when serving payloads by root during gossip, though potuz clarified that clients only need to serve gossip-received payloads and can drop range-synced ones.

The discussion concluded that while Nimbus should handle the range syncing part fine, they may need architectural adjustments for gossip and by-root requests. Tersec noted that the EIP heavily favors blinded schemas and essentially requires them, though potuz suggested that blinding might become obsolete after ePBS implementation since the default ePBS flow inherently resembles blinded operation.

**Participants:** 0xshum, m.kalinin, potuz, tersec, tomi0x
