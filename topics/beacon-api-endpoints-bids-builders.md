# Beacon API Endpoints for Bids and Builders

---

## 2026-03-31 (epbs)

The discussion centers around the current state and design of Beacon API endpoints needed for validator clients to act as builders in Ethereum's block building process. Potuz is looking for specifications to prepare bids that validator clients can sign, and while there are several pull requests in progress (PRs #552, #580, #584), none are merged yet. The main technical challenge is determining the proper API structure: whether endpoints should be in the `/validator` or `/builder` namespace, and how to handle the flow where beacon nodes cache payloads while builders specify parameters like gas limits and execution payments.

A key issue emerges around the existing `GET /eth/v1/validator/execution_payload_bid/{slot}/{builder_index}` endpoint, which doesn't allow validators to customize execution payment values, making it potentially unusable for external builders. Potuz identifies the need for two core builder namespace endpoints: one to get a bid to sign (with beacon node caching payload and fee recipient) and another to get an envelope to sign (either using cached payload or accepting new payload data). The discussion reveals that the "validator acting as builder" flow hasn't been fully implemented by any client yet, leading Potuz to continue with Prysm's internal gRPC implementation rather than waiting for the standardized Beacon API.

The conversation concludes with agreement that the current endpoints should be moved to a proper `/builder` namespace and made more useful for both external builders and consensus layer clients wanting to act as builders, though the specific implementation details remain unresolved as the broader builder API specification is still evolving.

**Participants:** bharath.vedartham, jameshe5018, nflaig, potuz
